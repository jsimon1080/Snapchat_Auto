...
def protoParse(schema, data, content_type):
    ...
    if content_type == 0:
        ...
        return (
            schema.Content.startMedia.unknown.unknown.unknown.cacheId,
            schema.Content.chat.mediatext.mediatext2.mediatextFinal,
        )
    else:
        return "Error", np.nan
...
def getChats(database):
    ...
    df_arroyo["message_content"], df_arroyo["extramessage_content"] = np.where(
        df_arroyo["content_type"] == 0,
        vect(schema, df_arroyo["message_content"], 0),
        np.where(
            df_arroyo["content_type"] == 1,
            vect(schema, df_arroyo["message_content"], 1),
            np.where(
                df_arroyo["content_type"] == 2,
                vect(schema, df_arroyo["message_content"], 2),
                "ERROR - Something went wrong when parsing this message.",
            ),
        ),
    )
...
th {
    ...
    html = html.replace(
        "<th>Creation Timestamp UTC+0</th>", '<th style="min-width: 180px;">Creation Timestamp UTC+0</th>'
    )
...
def main(snapchatFolder):
    ...
    final_df = df_chats[
        ((df_chats["content_type"] == 0) & (df_chats["isMedia"] == True))
        | (df_chats["content_type"] == 1)
        | ((df_chats["content_type"] == 2) & (df_chats["isMedia"] == True))
    ]
    final_df = final_df.merge(df_friends, on="userId")
    final_df = final_df.drop(columns="isMedia")
    final_df = final_df.rename(
        columns={
            "Creation Timestamp": "Creation Timestamp UTC+0",
            "Read Timestamp": "Read Timestamp UTC+0",
            "message_content": "Message Content",
            "server_message_id": "Server Message ID",
            "content_type": "Content Type",
            "extramessage_content": "Comment",
            "userId": "User ID",
        }
    )
