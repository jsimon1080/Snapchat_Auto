...
th {
    ...
    for index, clientConversationID in final_df.groupby("Client Conversation ID"):
        html = html + template % clientConversationID.to_html(
            classes=["table table-bordered table-striped table-hover table-xl table-responsive text-break w-auto"],
            escape=False,
            index=False,
        )

    html = html + template % friends_df.to_html(
        classes=["table table-bordered table-striped table-hover table-xl table-responsive text-wrap"],
        escape=False,
        index=False,
    )
    html = html + template % group_df.to_html(
        classes=["table table-bordered table-striped table-hover table-xl table-responsive text-wrap"],
        escape=False,
        index=False,
    )
    ...
    html = html.replace(
        "<th>Creation Timestamp UTC+0</th>", '<th style="min-width: 200px;">Creation Timestamp UTC+0</th>'
    )
            ...
            try:
                ...
                if kind.extension == "mp4":
                    return (
                        ...
                        + (relpath)
                        + '" type="video/mp4"> Your browser does not support the video tag. </video> <a href="'
                        + (relpath)
                        + '"><br>'
                        + basename
                        + "</a>"
                    ...
                    return (
                        '<a href="' + (relpath) + '"><img src="' + (relpath) + '" width="150" ><br>' + basename + "</a>"
                    )
    ...
    try:
        if os.path.exists(userPlist):
            ...
            with open(userPlist, "rb") as f:
                ...
                if uuid == None:
                    logger.info(
                        "No user found in User.plist! User might be logged out from Snapchat. Data will likely be incomplete."
                    )
                    return ""
            return uuid.group(0)
        else:
...
def getFriendsAppGroupPlistStorage(app_group_plist_storage_list, arroyo):
    ...
    for i in data1["GROUPS"]["NS.objects"]:  # Getting info on Chat groups
        try:
            group_name = i["GROUP_NAME"].encode("cp1252", "xmlcharrefreplace").decode("cp1252")
            group_id = i["GROUP_ID"]
        except Exception as Error:
    ...
    try:  # Getting info on Chat groups that does not have a set name (Might not be needed, will need more testing)
        ...
        logger.error(
            f"Error getting chat groups with no name, this is not expected but will not break anything major: {E}"
        )
    ...
    for index, row in df_group_noname.iterrows():
        try:
            ...
            logger.error(
                f"Error appending chat groups with no name, this is not expected but will not break anything major: {E}"
            )
    ...
    for i in data1["SECTIONS"]["NS.objects"]:
        ...
        for i in friends:
            if i["CODED_SUBTYPE"] == "SUBTYPE_GROUP":
                try:
                    group_name = (
                        i["GROUP_GROUP_NAME"]["NS.string"].encode("cp1252", "xmlcharrefreplace").decode("cp1252")
                    )
...
def getFriendsPrimary_DisplayMetadata(primary, arroyo):
    logger.info("")
    logger.info(
        f"Could not get friends from default location, trying from third location {ntpath.basename(primary)}(DisplayMetadata) and {ntpath.basename(arroyo)} (EXPERIMENTAL)"
    )
    ...
    try:
        ...
        private_conv = df_conversations[df_conversations["Conversation Type"] == "Private"].drop(
            columns=["Conversation Type"]
        )
        array = []
            ...
            try:
                ...
                if file in foundFiles:
                    ...
                    if os.stat(foundFiles[fileIndex]).st_size != 0 and kind is not None:
                        if (
                            kind.extension == "mp4"
                            or kind.extension == "jpg"
                            or kind.extension == "png"
    ...
    for index, row in df.iterrows():
        ...
        if messages == "":
            df.loc[index, "message_content"] = (
                """ERROR - Something went wrong when parsing this message. <br> Manually verify the message with Client Conversation ID and Server Message ID in arroyo.db"""
            )
            continue
        meddelande = ""
        try:
...
def mergeCacheChats(cache_df, chats_df, persistent_df, cache_arroyo_df):
    ...
    for index_arroyo, row_arroyo in cache_arroyo_df.iterrows():
        for index_chat, row_chat in chats_df.iterrows():
            if (
                row_chat["client_conversation_id"] == row_arroyo["client_conversation_id"]
                and row_chat["server_message_id"] == row_arroyo["server_message_id"]
            ):
                if not isinstance(row_arroyo["message_content"], float) and not isinstance(
                    row_arroyo["message_content"], bytes
                ):
        ...
        try:
            ...
            try:
                if str(row["SERVER_MESSAGE_ID_PART"]) == "nan":
                    ...
                    merge_df.loc[index, "server_message_id"] = (
                        str(row["server_message_id"]) + "." + str(row["SERVER_MESSAGE_ID_PART"])
                    )
            ...
            #     merge_df.loc[index, 'content_type'] = "Video (Unknown Source)"
                ...
                if row["content_type"] in [
                    "local_message_reference",
                    "Unknown .1020",
                ]:  # or row['content_type'] == 'Unknown .1020':
                    continue
                else:
    ...
    final_df = merge_df.rename(
        columns={
            "client_conversation_id": "Client Conversation ID",
            "server_message_id": "Server Message ID",
...
def main(Application, AppGroup, keychain):
    ...
    try:
        ...
        if len(arroyo) == 0:
            logger.info(
                "Could not find Chat database arroyo.db, this script does currently not support extractions without this database. Exiting"
            )
            os.system("pause")
            sys.exit()
        ...
        cacheController = glob.glob(
            snapchatFolder + "/Documents/global_scoped/cachecontroller/*cache_controller.db", recursive=True
        )
        if base_folder_share != "":
        ...
        try:
            contentmanager = glob.glob(
                snapchatFolder + f"/Documents/contentmanagerV3_{uuid_sha256}/contentManagerDb.db", recursive=True
            )[0]
        except:
            contentmanager = ""
        try:
            galleryEncrypteddb = glob.glob(
                snapchatFolder + f"/Documents/**/{uuid_sha256}/gallery.encrypteddb", recursive=True
            )[0]
        except:
            try:
                galleryEncrypteddb = glob.glob(
                    snapchatFolder + f"/Documents/**/{user_scoped_id}/gallery.encrypteddb", recursive=True
                )[0]
            except:
                galleryEncrypteddb = ""
        try:
    ...
    try:
        ...
        try:
            ...
            try:
                friends_df, group_df, df_snapchatter = getFriendsPrimary_DisplayMetadata(
                    Path(primaryDoc[0]), Path(arroyo[0])
                )
            except:
                logger.info("")
    ...
    if keychain_file != "" and scdb != "":
        df_merge = DecryptLocalMemories_iOS.main(
            galleryEncrypteddb, scdb, keychain_file, memories_cache_df, SCContentFolder
        )
